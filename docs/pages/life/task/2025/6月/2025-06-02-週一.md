...
- 確認是否可以做單頁當不同店家點擊之後顯示不同內容
  - 先找到前台原始碼
    - 了解專案架構在 ipetbooking-site
      - landing-frontend 是 愛寵訂 愛美訂 的前台
  
- 判斷住宿服務與外包拆分的地方
  - 先構出流程圖
    - 流程：切版 -> 基礎切版功能 -> API 串接 -> 功能邏輯實作 -> 測試 -> 上線
    - 切分：用功能部位切分，現在功能部位主要分為：基本設定、控房系統、特殊房價、加購服務、住宿服務問卷(暫定不做)
